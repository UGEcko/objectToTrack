...
optimSettings.performance_log = false
...
function processArray(filePath: string, track:string, returnProp: returnProperties, logObjectToFile?:string) {
  ...
  const [animatedPos]: KeyframesVec3 = optimizedPos; // Extract the animated position array
  const [animatedRot]: KeyframesVec3 = optimizedRot; // Extract the animated position array
...
if (validateRmmodel(envFile) == true) {
  const fuckass = new CustomEvent(time).animateTrack(gameTrack,durationOffset)
  fuckass.animate.position = processArray(envFile,track, Prop.Position,logObjectToFile)
  fuckass.animate.rotation = processArray(envFile,track,Prop.Rotation,logObjectToFile)
  fuckass.push()
} else {
